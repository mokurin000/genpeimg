...
static void _Noreturn show_usage(void)
{
	...
	while (argc-- > 0)
	{
		...
		case 'd':
			...
			while (*h != 0)
			{
				...
				case 'e':
					if (is_pos)
						set_pe_opt_hdr_dll_chara |= 0x20;
					else
						mask_pe_opt_hdr_dll_chara &= ~0x20;
					break;
				case 'd':
					if (is_pos)
						set_pe_opt_hdr_dll_chara |= 0x40;
					else
						mask_pe_opt_hdr_dll_chara &= ~0x40;
					break;
				case 'f':
					if (is_pos)
						set_pe_opt_hdr_dll_chara |= 0x80;
					else
						mask_pe_opt_hdr_dll_chara &= ~0x80;
					break;
				case 'n':
					if (is_pos)
						set_pe_opt_hdr_dll_chara |= 0x100;
					else
						mask_pe_opt_hdr_dll_chara &= ~0x100;
					break;
				case 'i':
					if (is_pos)
						set_pe_opt_hdr_dll_chara |= 0x200;
					else
						mask_pe_opt_hdr_dll_chara &= ~0x200;
					break;
				case 's':
					if (is_pos)
						set_pe_opt_hdr_dll_chara |= 0x400;
					else
						mask_pe_opt_hdr_dll_chara &= ~0x400;
					break;
				case 'b':
					if (is_pos)
						set_pe_opt_hdr_dll_chara |= 0x800;
					else
						mask_pe_opt_hdr_dll_chara &= ~0x800;
					break;
				case 'a':
					if (is_pos)
						set_pe_opt_hdr_dll_chara |= 0x1000;
					else
						mask_pe_opt_hdr_dll_chara &= ~0x1000;
					break;
				case 'w':
					if (is_pos)
						set_pe_opt_hdr_dll_chara |= 0x2000;
					else
						mask_pe_opt_hdr_dll_chara &= ~0x2000;
					break;
				case 'c':
					if (is_pos)
						set_pe_opt_hdr_dll_chara |= 0x4000;
					else
						mask_pe_opt_hdr_dll_chara &= ~0x4000;
					break;
				case 't':
					if (is_pos)
						set_pe_opt_hdr_dll_chara |= 0x8000;
					else
						mask_pe_opt_hdr_dll_chara &= ~0x8000;
					break;
				default:
